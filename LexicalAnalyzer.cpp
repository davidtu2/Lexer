...
{
	lexState = 1;
	myOutputFile.open("TokenLexemeList.txt");
...
void LexicalAnalyzer::lexer() {
	//while condition ensures that I'm still in the file
	while (myReadFile.get(ch)) {
		...
		//lookups for $$, :=, /=, <=, => and [*
			...
			if (ch == '*') {
				//the lexeme and ch passed are '[' and '*', respectively, but they will be overwritten by the algoritm
				commentSkip(lexeme);
			}
			else {
				//Case: Illegal lexeme, [
...
void LexicalAnalyzer::runIDDFSM() {
	...
	//keep iterating until pointer finds a illegal char. The myReadFile condition prevents infinite loop when the pointer passes End of File
	while ((isalpha(ch) | ch == '_') && myReadFile) {
		if (isalpha(ch)) {
			column = 0;
			//lexState = 3
			lexState = IDDFSM[lexState][column];
	...
	if (lexState == 2 | lexState == 3) {
		//Note: Keyword check is case-sensitive
		...
		//lexeme ends with an underscore. This is lexState = 4 scenario
		token = "Unknown";
	}
